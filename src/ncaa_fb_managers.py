...
class BaseNCAAFBManager:
    ...
    logger = logging.getLogger('NCAAFB')
    ...
    def __init__(self, config: Dict[str, Any], display_manager: DisplayManager, cache_manager: CacheManager):
        ...
        self.ncaa_fb_config = config.get("ncaa_fb_scoreboard", {})
        ...
        self.logo_dir = self.ncaa_fb_config.get("logo_dir", "assets/sports/ncaa_fbs_logos")
    ...
    def _fetch_odds(self, game: Dict) -> None:
        ...
        # Removed favorites_only check as we now filter for top 25 games upstream
    ...
    def _fetch_ncaa_fb_api_data(self, use_cache: bool = True) -> Optional[Dict]:
        """
        Fetches the schedule for the current week's NCAAFB games, caches it, and filters for relevant games.
        """
        now = datetime.now(pytz.utc)
        current_year = now.year if now.month >= 8 else now.year - 1
        date_str = now.strftime('%Y%m%d')
        # Fetch today's scoreboard to get current week
        cache_key_week = f"ncaafb_current_week_{current_year}_{date_str}"
        current_week = self.cache_manager.get(cache_key_week)
        if current_week is None:
            url_today = f"{ESPN_NCAAFB_SCOREBOARD_URL}?dates={date_str}&seasontype=2"
            try:
                response = self.session.get(url_today, headers=self.headers, timeout=15)
                response.raise_for_status()
                data = response.json()
                if 'events' in data and data['events']:
                    current_week = data['events'][0].get('week', {}).get('number', 1)
                else:
                    current_week = 1  # Default to 1 if no events
                self.cache_manager.set(cache_key_week, current_week, ttl=86400)
                self.cache_manager.set(cache_key, events, ttl=3600)
                self.logger.info(f"[NCAAFB] Determined current week: {current_week}")
            except Exception as e:
                self.logger.error(f"[NCAAFB] Error fetching current week: {e}")
                current_week = 1
    ...
    # Rest of the method remains unchanged...        
        # Now fetch the full week
        cache_key = f"ncaafb_schedule_{current_year}_week_{current_week}"
        if use_cache:
            cached_data = self.cache_manager.get(cache_key, max_age=3600)  # 1 hour cache for active week
            if cached_data:
                self.logger.info(f"[NCAAFB] Using cached schedule for {current_year} week {current_week}")
                return {'events': cached_data}
        
        self.logger.info(f"[NCAAFB] Fetching {current_year} week {current_week} schedule from ESPN API...")
        try:
            url = f"{ESPN_NCAAFB_SCOREBOARD_URL}?seasontype=2&week={current_week}&dates={current_year}"
            ...
            self.logger.info(f"[NCAAFB] Successfully fetched and cached {len(events)} events for {current_year} week {current_week}.")
            return {'events': events}
        except requests.exceptions.RequestException as e:
            self.logger.error(f"[NCAAFB] API error fetching schedule for {current_year} week {current_week}: {e}")
            return None
    ...
    def _load_and_resize_logo(self, team_abbrev: str) -> Optional[Image.Image]:
        ...
        # Map problematic abbreviations to safe filenames
        abbr_to_filename = {
            "TA&M": "TAMU"  # Map TA&M to TAMU.png
            # Add other mappings as needed, e.g., "ULL": "Louisiana"
        }
        safe_abbrev = abbr_to_filename.get(team_abbrev, team_abbrev)
        if safe_abbrev in self._logo_cache:
            return self._logo_cache[safe_abbrev]
    
        logo_path = os.path.join(self.logo_dir, f"{safe_abbrev}.png")
    ...
    def _extract_game_details(self, game_event: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract ranks
            home_rank = home_team.get("curatedRank", {}).get("current", 99)
            away_rank = away_team.get("curatedRank", {}).get("current", 99)
            home_rank = home_rank if home_rank <= 25 else None
            away_rank = away_rank if away_rank <= 25 else None
...
class NCAAFBLiveManager(BaseNCAAFBManager): # Renamed class
    ...
    def __init__(self, config: Dict[str, Any], display_manager: DisplayManager, cache_manager: CacheManager):
        ...
        self.game_display_duration = self.ncaa_fb_config.get("live_game_duration", 10)
                ...
                if data and "events" in data:
                    for event in data["events"]:
                        ...
                        if details and (details["is_live"] or details["is_halftime"]):
                            # Filter for top 25 games
                            if details["home_rank"] or details["away_rank"]:
                                if self.show_odds:
                    ...
                    if should_log:
                        if new_live_games:
                            self.logger.info(f"[NCAAFB] Found {len(new_live_games)} live/halftime games involving top 25 teams.")
                            ...
                            self.logger.info(f"[NCAAFB] No live/halftime games found involving top 25 teams.")
    ...
    def _draw_scorebug_layout(self, game: Dict, force_clear: bool = False) -> None:
        ...
        try:
            ...
            rank_font = self.fonts['detail']
            if game.get("away_rank"):
                rank_text = f"#{game['away_rank']}"
                rank_x = 2
                rank_y = self.display_height - 10  # above timeouts
                self._draw_text_with_outline(draw_overlay, rank_text, (rank_x, rank_y), rank_font, fill=(255, 255, 0))
            if game.get("home_rank"):
                rank_text = f"#{game['home_rank']}"
                rank_width = draw_overlay.textlength(rank_text, font=rank_font)
                rank_x = self.display_width - rank_width - 2
                rank_y = self.display_height - 10
                self._draw_text_with_outline(draw_overlay, rank_text, (rank_x, rank_y), rank_font, fill=(255, 255, 0))
...
class NCAAFBRecentManager(BaseNCAAFBManager): # Renamed class
    ...
    def __init__(self, config: Dict[str, Any], display_manager: DisplayManager, cache_manager: CacheManager):
        ...
        self.last_game_switch = 0
        self.game_display_duration = 10 # Display each recent game for 10 seconds
    ...
    def update(self):
        ...
        try:
            ...
            # Process games and filter for final games & top 25
            processed_games = []
            for event in events:
                game = self._extract_game_details(event)
                # Filter criteria: must be final and involve top 25
                if game and game['is_final'] and (game["home_rank"] or game["away_rank"]):
                    processed_games.append(game)

            # Sort by game time, most recent first
            processed_games.sort(key=lambda g: g.get('start_time_utc') or datetime.min.replace(tzinfo=timezone.utc), reverse=True)
            
            # Limit to the specified number of recent games
            team_games = processed_games[:self.recent_games_to_show]
            ...
            if new_game_ids != current_game_ids:
                self.logger.info(f"[NCAAFB Recent] Found {len(team_games)} final games within window involving top 25 teams for display.") # Changed log prefix
    ...
    def _draw_scorebug_layout(self, game: Dict, force_clear: bool = False) -> None:
        ...
        try:
            ...
            if game.get("away_rank"):
                rank_text = f"#{game['away_rank']}"
                rank_x = 2
                rank_y = self.display_height - 10  # above records if shown
                self._draw_text_with_outline(draw_overlay, rank_text, (rank_x, rank_y), rank_font, fill=(255, 255, 0))
            if game.get("home_rank"):
                rank_text = f"#{game['home_rank']}"
                rank_width = draw_overlay.textlength(rank_text, font=rank_font)
...
class NCAAFBUpcomingManager(BaseNCAAFBManager): # Renamed class
    ...
    def __init__(self, config: Dict[str, Any], display_manager: DisplayManager, cache_manager: CacheManager):
        ...
        self.last_game_switch = 0
        self.game_display_duration = 10 # Display each upcoming game for 10 seconds
    ...
    def update(self):
        ...
        try:
            ...
            for event in events:
                game = self._extract_game_details(event)
                # Filter criteria: must be upcoming ('pre' state) and involve top 25
                if game and game['is_upcoming'] and (game["home_rank"] or game["away_rank"]):
                    if self.show_odds:
                        self._fetch_odds(game)
                    processed_games.append(game)

            # Summary logging instead of verbose debug
            self.logger.info(f"[NCAAFB Upcoming] Found {len(processed_games)} upcoming games involving top 25 teams")

            # Sort by game time, earliest first
            processed_games.sort(key=lambda g: g.get('start_time_utc') or datetime.max.replace(tzinfo=timezone.utc))
            
            # Limit to the specified number of upcoming games
            team_games = processed_games[:self.upcoming_games_to_show]
